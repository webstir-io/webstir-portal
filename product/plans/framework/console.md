...
4. **Automation**
   - The framework console exposes `packages` subcommands that rebuild, verify, and publish bundles; the helper script now shells into `dotnet run --project framework/Framework.csproj -- packages …`.
   - `.github/workflows/ci.yml` uses `dotnet run --project framework/Framework.csproj -- packages verify`.
   - `.github/workflows/release.yml` uses `dotnet run --project framework/Framework.csproj -- packages sync --publish --verify`.
...
- `framework/Packaging/PackageBuilder.cs`
  ...
  - Consumed by the framework console command (`PackageConsoleCommand`) to service `framework packages ...`.
