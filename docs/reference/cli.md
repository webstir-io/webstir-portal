...
- Ensure `.npmrc` is configured with credentials (when publishing, provide `NPM_TOKEN`) so registry operations succeed.
...
- Requires local access to the framework packages; configure `.npmrc` for npmjs registry installs.
