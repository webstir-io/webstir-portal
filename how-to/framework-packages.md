...
- The separate `framework` CLI (see `FrameworkCLI/`) rebuilds those bundles, copies the tarballs into `framework/Resources/tools` (embedded with the CLI) and `framework/out` (the local package repository), and refreshes `framework/out/manifest.json` with new hashes.
...
2. **Rebuild packages** – Run `dotnet run --project FrameworkCLI -- build` from the repo root (or use the `framework` executable if you have published it).
...
- The workflow:
  ...
  2. Runs `dotnet run --project FrameworkCLI -- build --publish --verify` to rebuild tarballs, republish packages (if missing), and confirm hashes.
...
## Developer Helpers
- Run `utilities/build-packages.sh` before committing frontend/testing changes. The helper checks for tracked changes under `framework/frontend` or `framework/testing`, calls `framework build` when needed, and stages the updated `framework/out/manifest.json` for you.
...
- Optionally, run `framework verify` to ensure `framework/out` and `framework/Resources/tools` remain unchanged (useful in CI after a clean checkout).
- `./utilities/format-build.sh` already includes a `framework build --verify` check, so running that helper before handing off guarantees manifests and tarballs are current.
- The manifest records the UTC generation timestamp; `framework verify` recomputes tarball hashes to confirm the manifest matches what is on disk.
