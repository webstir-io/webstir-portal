...
- `Framework/Frontend` and `Framework/Testing` contain the source for the bundled `@webstir-io/webstir-frontend` and `@webstir-io/webstir-test` packages.
...
- The installer pins `@webstir-io/webstir-frontend` and `@webstir-io/webstir-test` to `file:./.webstir/<tarball>.tgz`, then ensures `node_modules` contains the expected versions.
