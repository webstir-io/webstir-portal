...
- `webstir packages sync` rebuilds the frontend and testing packages and copies the tarballs into both `framework/Resources/tools` (embedded with the CLI) and `framework/out` (the local package repository).
...
2. Run `webstir packages sync` from the repo root.
   ...
   - Use `webstir packages publish` (or `sync --publish`) after the rebuild to push the new tarballs to GitHub Packages if the version is missing. Ensure `GH_PACKAGES_TOKEN` is exported so npm can authenticate. Tarballs generated by `sync` are ignored by git; CI regenerates them during releases.
   - For CI enforcement, run `webstir packages sync --verify` (or `webstir packages verify`) to ensure the manifest and tarballs are committed.
...
- Optionally, run `webstir packages verify` to ensure `framework/out` and `framework/Resources/tools` remain unchanged (useful in CI after a clean checkout).
- `./utilities/format-build.sh` already includes a `webstir packages sync --verify` check, so running that helper before handing off guarantees manifests and tarballs are current.
- Manifests record the generation timestamp and commit hash; `webstir packages verify` ensures they match `git rev-parse HEAD` before passing.
